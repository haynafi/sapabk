...
function applyFilters() {
    ...
    router.get(route('admin.audit-logs'), params, {
        preserveState: true,
        preserveScroll: true,
    });
}
...
<template>
    <Head :title="t('admin.all_audit_logs')" />
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                    <div class="p-4 sm:p-6">
                        ...
                        <!-- Filters -->
                        <div
                            class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4"
                            :class="{ 'hidden sm:grid': !showFilters }"
                        >
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('audit.user') }}</label>
                                <select
                            ...
                            <div class="flex items-end gap-2">
                                <button
                                    @click="applyFilters"
                                    class="flex-1 px-4 py-3 min-h-[44px] bg-indigo-600 text-white rounded-md hover:bg-indigo-700"
                                >
                                    {{ t('common.filter') }}
                                </button>
                                <button
                                    @click="clearFilters"
                                    class="flex-1 px-4 py-3 min-h-[44px] bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200"
                                >
                                    {{ t('common.clear') }}
                                </button>
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="log in logs.data" :key="log.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="text-sm text-gray-500">{{ log.user?.school?.name || '-' }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                        ...
                        <div v-if="logs.data.length > 0" class="md:hidden space-y-4">
                            ...
                            >
                                ...
                                <div class="text-xs text-gray-400">
                                    ...
                                    <span v-if="log.user?.school?.name"> ({{ log.user.school.name }})</span>
                                </div>
                            </div>
                        </div>
